...
class SkopeGeometry(metaclass=ABCMeta):
    def extract(self, dataset: DatasetReader, zonal_statistic: ZonalStatistic, band_range: Sequence[int]):
        self.validate_geometry(dataset)
        return self.extract_raster_slice(dataset, zonal_statistic, band_range)

    @abstractmethod
    def extract_raster_slice(self, dataset: DatasetReader, zonal_statistic: ZonalStatistic, band_range: Sequence[int]): pass

    @abstractmethod
    def validate_geometry(self, dataset: DatasetReader): pass
...
class SkopePointModel(Point, SkopeGeometry):
    ...
    def extract_raster_slice(self,
                dataset: DatasetReader,
                zonal_statistic: ZonalStatistic,
                band_range: Sequence[int]):
        logger.info('extracting point: %s', self)
        row_index, column_index = dataset.index(self.coordinates[0], self.coordinates[1])
        logging.info('indices: %s', (row_index, column_index))
        data = dataset.read(list(band_range), window=windows.Window(column_index, row_index, 1, 1), out_dtype=np.float64).flatten()
...
class BaseSkopePolygonModel(SkopeGeometry):
    ...
    def extract_raster_slice(self,
        ...
        masked, transform, window = mask.raster_geometry_mask(dataset, shapes=[self], crop=True, all_touched=True)
