...
async def extract_timeseries(timeseries_request: TimeseriesRequest, dataset_manager: DatasetManager, output: Dict, *, task_status: TaskStatus = TASK_STATUS_IGNORED):
    task_status.started()
    ...
    with rasterio.Env():
        with rasterio.open(metadata.path) as ds:
            ...
            transform_xs = timeseries_request.extract_slice(ds, band_range=band_range_transform)['data'] if band_range_transform else None
